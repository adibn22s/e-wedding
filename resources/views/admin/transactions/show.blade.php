                ...
                <div class="flex flex-row gap-x-10">
                    ...
                    <div class="flex flex-col gap-y-10">    
                        <div>
                            ...
                            <h3 class="text-indigo-950 text-xl font-bold">Rp {{ $book->total_amount }}</h3>
                        </div>
                        <div>
                            @if($book->is_paid)
                        ...
                        <div>
                            ...
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $book->created_at }}</h3>
                        </div>
                        <div>
                            <p class="text-slate-500 text-sm">Book Date</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $book->book_date }}</h3>
                        </div>
                        <div class="">
                            <p class="text-slate-500 text-sm">Customer</p>
                            <h3 class="text-indigo-950 text-xl font-bold">{{ $book->user->name }}</h3>
                        </div>
                    </div>
                    <div>
                        <img src="{{ Storage::url($book->proof) }}" alt="">
                    </div>
                </div>
                @if($book->is_paid)
                @else
                <hr class="my-5">
                <form action="{{ route('admin.catalogue.approve_transactions', $book->id) }}" method="POST">
                    @csrf
                    @method('PUT')
